...
public class Game {
	...
	public void assemblePieces() {
		...
		Armored a1 = new Armored(player1, this, "Squanchy");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(a1);
		Armored a2 = new Armored(player2, this, "Gear Head");
		board[p2InitRow][ind2.remove(0)].setPiece(a2);

		Medic m1 = new Medic(player1, this, "Mr. Poopy Butthole");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(m1);

		Medic m2 = new Medic(player2, this, "Jessica");
		board[p2InitRow][ind2.remove(0)].setPiece(m2);

		Ranged r1 = new Ranged(player1, this, "Bird Person");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(r1);

		Ranged r2 = new Ranged(player2, this, "Phoenix Person");
		board[p2InitRow][ind2.remove(0)].setPiece(r2);

		Speedster sp1 = new Speedster(player1, this, "Krombopulos Michael");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(sp1);

		Speedster sp2 = new Speedster(player2, this, "Tammy");
		board[p2InitRow][ind2.remove(0)].setPiece(sp2);

		Super su1 = new Super(player1, this, "Jacked Morty");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(su1);

		Super su2 = new Super(player2, this, "King JellyBean");
		board[p2InitRow][ind2.remove(0)].setPiece(su2);

		Tech t1 = new Tech(player1, this, "Rick");
		board[p1InitRow + 1][ind1.remove(0)].setPiece(t1);

		Tech t2 = new Tech(player2, this, "Evil Morty");
		...
		for (int j = 0; j < boardWidth; j++) {
			board[p1InitRow][j].setPiece(new SideKickP1(this, "Mr. Meeseeks "+(j+1) ));
			board[p2InitRow + 1][j].setPiece(new SideKickP2(this, "Gazorpazorp "+ (j+1)));
		}
		for (int i = 0; i < board.length; i++)
